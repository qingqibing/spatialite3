...
public class Database {
    ...
    void exec(String sql, jsqlite.Callback cb) throws jsqlite.Exception {
        synchronized (this) {
            _exec(sql, cb);
        }
    }
    ...
    private TableResult get_table(String sql, int maxrows) throws jsqlite.Exception {
    ...
    private TableResult get_table(String sql, int maxrows, String args[]) throws jsqlite.Exception {
        TableResult ret = new TableResult(maxrows);
        if (!is3()) {
            try {
                exec(sql, ret, args);
    ...
    TableResult get_table(String sql, String args[]) throws jsqlite.Exception {
    ...
    synchronized static boolean complete(String sql) {
    ...
    private void set_last_error(int error_code) {
    ...
    void set_encoding(String enc) throws jsqlite.Exception {
        synchronized (this) {
            _set_encoding(enc);
        }
    }
    ...
    private Vm compile(String sql) throws jsqlite.Exception {
        synchronized (this) {
            Vm vm = new Vm();
            vm_compile(sql, vm);
            return vm;
    ...
    private Vm compile(String sql, String args[]) throws jsqlite.Exception {
        synchronized (this) {
            Vm vm = new Vm();
    ...
    void key(String skey) throws jsqlite.Exception {
        synchronized (this) {
            byte ekey[] = null;
    ...
    void rekey(String skey) throws jsqlite.Exception {
        synchronized (this) {
            byte ekey[] = null;
            if (skey != null && skey.length() > 0) {
                ekey = new byte[skey.length()];
    ...
    private static long long_from_julian(double d) {
        d -= 2440587.5;
        d *= 86400000.0;
        return (long) d;
    }
